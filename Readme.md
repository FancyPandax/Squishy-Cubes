Squishy Cubes




Squishy Cubes is a 2D soft-body physics simulation built in C++ using Raylib
.
It demonstrates Verlet integration, stick constraints, and shape-matching physics to create interactive, squishy cubes.

Drag, flick, and manipulate cubes with your mouse, and adjust physics parameters in real-time using on-screen sliders.

Demo


[GIF: Shows cubes being dragged, flicked, and colliding with each other.](demos\Cubes Demo.gif)

Features

Soft-body cubes with realistic squishy physics

Verlet integration for stable particle motion

Stick constraints for edge connections

Quad-based shape matching to preserve cube shape

Mouse interaction:

Drag cubes

Flick cubes with velocity

Cube-cube and particle collision

Real-time sliders for physics parameters

Add/Delete cubes:

A – Add a new cube at mouse position

D – Delete cube under mouse

Controls
Action	Key / Mouse
Grab / Drag a particle	Left Mouse
Flick cube	Drag + Release
Add cube	A
Delete cube under mouse	D
Adjust physics sliders	Mouse
Download & Run (Windows)

You can try Squishy Cubes without compiling by downloading the pre-compiled executable from the latest GitHub release:

Download main.exe

Instructions:

Download main.exe from the release.

Place it in any folder.

Double-click main.exe to run.

Enjoy the simulation!

 Only works on Windows.

Future Improvements

Better cube-cube collision for deformed cubes

Stress visualization for quads

Optimized particle collisions for larger cubes

Multiple selection or stretchable shapes

License

This project is licensed under the MIT License – see LICENSE
 for details.

Author

Asher Wolf 
